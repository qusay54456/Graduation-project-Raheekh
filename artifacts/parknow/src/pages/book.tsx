...
export default function Book() {
  ...
  const createReservation = useCreateReservation({
    mutation: {
      ...
      onError: (err: any) => {
        toast({
          title: "خطأ في الحجز",
          description: err.error || "Failed to create reservation",
          variant: "destructive",
        });
      }
    }
  ...
  const handleBook = () => {
    ...
    const now = new Date();
    // Start at the next full hour or now if immediate, let's just use now
  ...
  if (!lot) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <h2 className="text-2xl font-bold">الموقف غير موجود (Lot not found)</h2>
        <Button className="mt-4" onClick={() => setLocation("/")}>العودة للرئيسية (Back to Home)</Button>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-primary mb-2">{lot.name}</h1>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                المواقف الخضراء متاحة للحجز (Green spots are available)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
                {spots?.map((spot) => (
                  <button
                    ...
                    className={`
                      relative flex items-center justify-center h-16 rounded-md border-2 font-mono text-lg font-bold transition-all
              ...
              <div className="flex items-center justify-center gap-6 mt-8 text-sm">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-primary border-2 border-primary text-primary-foreground flex items-center justify-center"></div>
                  <span>محدد (Selected)</span>
                </div>
                <div className="flex items-center gap-2">
        ...
        <div className="space-y-4">
          <Card className="sticky top-20">
            <CardHeader>
              <CardTitle>تفاصيل الحجز (Details)</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label>الموقف المحدد (Selected Spot)</Label>
              ...
              <div className="space-y-2">
                ...
                <Select value={durationHours} onValueChange={setDurationHours}>
                  ...
                  <SelectContent dir="rtl">
                    <SelectItem value="1">ساعة واحدة (1 Hour)</SelectItem>
                    <SelectItem value="2">ساعتان (2 Hours)</SelectItem>
                    <SelectItem value="3">3 ساعات (3 Hours)</SelectItem>
                    <SelectItem value="4">4 ساعات (4 Hours)</SelectItem>
                    <SelectItem value="6">6 ساعات (6 Hours)</SelectItem>
                    <SelectItem value="12">نصف يوم (12 Hours)</SelectItem>
                    <SelectItem value="24">يوم كامل (24 Hours)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </CardContent>
            <CardFooter>
              <Button 
                className="w-full" 
                size="lg" 
                disabled={!selectedSpotId || createReservation.isPending}
                onClick={handleBook}
              >
