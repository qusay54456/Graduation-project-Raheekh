...
import { CheckCircle2, Wallet, ListChecks, Loader2 } from "lucide-react";
...
export default function PaymentSuccess() {
  ...
  useEffect(() => {
    ...
    setBooking(data);
    // Trigger the green checkmark animation on the next frame.
        ...
        <CardContent className="pt-10 pb-8 px-6 text-center space-y-6">
          ...
          <div className="flex justify-center">
            <div
              ...
              className={`relative h-24 w-24 rounded-full flex items-center justify-center transition-all duration-700 ${
            ...
            >
              ...
              />
              <CheckCircle2
                className={`h-16 w-16 text-secondary transition-all duration-500 ${
          ...
          <div className="space-y-2">
            <h1 className="text-2xl md:text-3xl font-bold text-primary" data-testid="text-success-title">
          ...
          <div className="inline-block bg-muted rounded-lg px-6 py-3">
            ...
            <div className="text-3xl font-mono font-bold text-primary mt-1" data-testid="text-confirmation-number">
          ...
          {qrSrc && (
            <div className="flex flex-col items-center gap-2">
              <img
                src={qrSrc}
                alt="Booking QR code"
                className="rounded-lg border-4 border-card shadow-md"
                width={220}
                height={220}
          ...
          <div className="rounded-lg bg-muted/40 px-4 py-3 text-sm space-y-1 max-w-sm mx-auto text-left rtl:text-right">
            ...
            <SummaryLine
              ...
              value={`${booking.durationHours} ${t("book.durationSuffix")}`}
            />
            <SummaryLine
              label={t("payment.total")}
              value={`${booking.totalPrice} ${t("common.currency")}`}
              bold
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
            <Button onClick={handleAddToWallet} variant="outline" data-testid="button-add-to-wallet">
              <Wallet className="mx-2 h-4 w-4" /> {t("payment.addToWallet")}
            </Button>
            <Button onClick={() => setLocation("/my-bookings")} data-testid="button-view-bookings">
              <ListChecks className="mx-2 h-4 w-4" /> {t("payment.viewBookings")}
            </Button>
          </div>
        </CardContent>
      </Card>
